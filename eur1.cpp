...
void eur_1(int *arr,int *eur1)
{
	int grade=0,j=0;
	int prof;
	for(int i=0;i<55;i++)
	{
		if(clist[i].type!=2) eur1[i]=arr[i];
		else eur1[i]=0;	    
	}
	...
	for(int i=0;i<55;i++)
	{
		if(eur1[i]==1)
		{
			prof=clist[i].pro_num;
			for(j=0;j<3;j++)
			{
					if(clist[clist[i].prac[j]-1].pro_num==prof) grade++;
					if(clist[clist[i].prac[j]-1].day==clist[i].day)
						if((clist[clist[i].prac[j]-1].start>clist[i].start && clist[clist[i].prac[j]-1].start<clist[i].end) ||  (clist[clist[i].prac[j]-1].end>clist[i].start && clist[clist[i].prac[j]-1].end<clist[i].end) || (clist[clist[i].prac[j]-1].start<=clist[i].start && clist[clist[i].prac[j]-1].end>=clist[i].end))
							grade++;
			}	
			if(clist[i].rel==0) grade++;
			if(clist[i].must!=0) grade++;
			if(grade!=0) eur1[i]=grade;	
			for(int j=0;j<3;j++)
			{
