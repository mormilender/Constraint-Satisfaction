...
void unplace(course* c)
{
	for(int i=c->start-8;i<c->end-8;i++)
	    cal[i][c->day]=NULL;
	if(c->rel>0)
		clist[c->rel-1].taken=0;
	if(c->rel2>0)
		clist[c->rel2-1].taken=0;
	c->taken=0;
	for(int j=0;j<3;j++)
		{
			if(c->prev_cource[j]>0)
				{
				   if(clist[c->prev_cource[j]-1].taken==0 && clist[c->prev_cource[j]-1].must==c->num+1)
						{
							clist[c->prev_cource[j]-1].must=0;
							must--;
						}
				}
		}
...
bool arc_3(int i,int *arc,vector <int> &prof,int points)
{
	int t_p=0,flag=0;
	vector <string> checked;
	if(points+clist[i].points==P) return 1;
	if(points+clist[i].points>P)return 0;
	if(points+clist[i].points<P)
	{
		place(&clist[i]);
		...
		for(int j=0;j<55;j++)
		{
			if(!checked.empty())
			{
			for(k=checked.begin();k<checked.end();k++)
				if(clist[j].name==*k)
					flag=1;
			}
			if(arc[j]==1 && flag==0)
			{
				t_p+=clist[j].points;
				checked.push_back(clist[j].name);
			}
			flag=0;
		}
		t_p+=points;
...
bool build(int points,int num_gen,vector <int> &prof)
{
	...
	if(for_check(prof,arr)==0 )return 0;
        for(i=0;i<55;i++)
        {
			if(arr[i]!=0 && clist[i].type!=2)
			{
                if(arc_3(i,arc,prof,points))
                {
				  place(&clist[i]);
				  ...
				  if(clist[i].type==1 || clist[i].type==4)
				  {
					  ...
					  { 
							...
							{
								if(arc_3(clist[i].prac[j]-1,arc,prof,points))
								{
