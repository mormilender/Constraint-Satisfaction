...
course* cal[8][5]={NULL};
course clist[55]={"c#",4,1,36,8,11,3,49,50,51,26,34,25,0,3,0,0,0,//courses list
...
int for_check(vector <int> &prof,int *arr)//forword check euristic
{
	int sum=0;
	for(int j=0;j<55;j++)
	{
...
int find_min(int *eur3)
{
	int j;
	int min=56;
	for(int i=0;i<55;i++)
	{
		if(eur3[i]<=min && eur3[i]!=-1)
		{
			min=eur3[i];
			j=i;
		}
...
void eur_3(int *arr,int *eur3)
{
	int j=0;
	int prof;
	for(int i=0;i<55;i++)
	{
		if(arr[i]!=0 && clist[i].type!=2) eur3[i]=0;
		else eur3[i]=-1;	    
	}
